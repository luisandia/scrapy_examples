...
class JobsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for listing in listings:
            ...
            yield scrapy.Request(link, callback=self.parse_listing,
                                 meta={
                                     'date': date, 'link': link, 'text': text
                                 })

        next_page_url=response.css("a.button.next::attr('href')")[-1].extract()
        # next_page_url = response.xpath('//a[text()="next > "]/@href')[-1].extract()
    ...
    def parse_listing(self, response):
        ...
        type = compensation = response.xpath(
